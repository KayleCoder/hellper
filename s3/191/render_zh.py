...
ERASE_OVERRIDE = {
    # 030.0 clustered the speech bubble together with the 부들부들 sfx below it;
    # clamp the erase to the bubble so the artwork underneath survives.
    '030.0': [(185, 328, 480, 652)],
}
MASK_ERASE = set()
erase_log = {}
# blocks whose lettering is far larger than a speech bubble (title cards, big sfx)
FONT_MAX = {
    # decorative title bands: the default 60px cap leaves them far too small
    # next to the original lettering, so raise it per label.
    '003.0': 92,  '070.0': 92,  '023.1': 128, '026.0': 104,
    '057.0': 140, '073.1': 120, '076.2': 150, '017.0': 140,
    '039.1': 150, '005.1': 140, '004.0': 108, '005.0': 108,
    '081.2': 128, '092.6': 92,  '102.7': 74,  '108.0': 68,
    '038.3': 68,  '038.1': 72,  '037.0': 78,  '105.1': 40,
    '105.4': 40,  '105.7': 40,  '034.3': 96,  '057.1': 110,
}
# text the OCR missed entirely, or clusters it merged wrongly: page -> [(rect, ko)]
EXTRA = {
    # clusters that merged a speech bubble with a full-panel sfx: hand-split so the
    # erase only ever touches the bubble, never the artwork.
    '022': [((448, 972, 822, 1138), '그래... 마지막 기회다 이거지..')],
    '053': [((325, 1018, 778, 1170), '이런 샹!!')],
    '081': [((242, 2022, 909, 2440), '전력변형 「신궁」')],
    '092': [((233, 960, 480, 1232), '간지러 샹!!'),
            ((262, 1314, 829, 1594), '워배트 STImode')],
    # stylised display lettering Vision could not read at all, found by a
    # colour-label sweep + a 2x re-OCR of the erased pages.
    '034': [((248, 368, 438, 768), '전부 뚫어 주마!!!!')],
    '057': [((262, 752, 792, 1000), 'FULL 모자이크')],
    # rect kept well inside the round bubble so the +8px pad never crosses its outline
    '103': [((708, 803, 788, 875), '응.')],
}
zh = {}
