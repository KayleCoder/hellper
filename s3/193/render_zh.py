...
ERASE_OVERRIDE = {
    # EXTRA 块默认加 24px pad，靠近画格边框/艺术时用 override 精确擦除
    # 气泡上窄下宽，分行擦除才不会吃掉尖角轮廓
    '038.4': [(185, 1668, 570, 1838), (185, 1838, 785, 2048)],
    '044.2': [(560, 85, 835, 195)],
    '044.3': [(515, 246, 875, 795)],
    '047.2': [(280, 1948, 478, 2008)],
    '078.3': [(618, 762, 846, 896)],
}
MASK_ERASE = set()
erase_log = {}
FONT_MAX = {
    '031.1': 140, '053.0': 112, '059.0': 96,  '080.0': 66,  '081.1': 130,
    '027.0': 80,  '038.4': 150, '037.4': 90,  '044.2': 90, '044.3': 76,  '036.2': 62,
    '009.0': 66,  '011.0': 68,  '016.2': 62,  '017.0': 62,  '039.0': 64,
    '003.1': 80,  '049.1': 52, '047.2': 44, '078.3': 64,
}
# 把对白气泡和满格特效字聚在一起的块，手工拆开
EXTRA = {
    '009': [((270, 1845, 440, 1990), '오오는 무슨-')],
    '037': [((245, 1105, 548, 1382), '좋았어~! 알려주지!!'),
            ((205, 1465, 578, 2260), "너! 령의 힘인 '형'을 갖추는 '사인'에 대해 생각해 본 적 있어??")],
    '038': [((268, 1378, 522, 1606), '해야 되냐?'),
            ((185, 1668, 785, 2048), '당연하지!!')],
    # 气泡里有彩色冲击碎片穿插在两段文字之间，必须分开擦
    '047': [((280, 1948, 478, 2008), '바보! ㅋ')],
    '078': [((618, 762, 846, 896), '..어?')],
    '044': [((560, 85, 835, 195), '그렇지!!'),
            ((515, 246, 875, 795), '머리는 너가 저 백정 놈보다 훨씬 낫구나!')],
}
zh = {}
