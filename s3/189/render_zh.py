...
ERASE_OVERRIDE = {
    '013.0': [(530, 100, 736, 256)],
    '026.0': [(150, 2148, 800, 2360)],   # 黑底白字番号带，用框内中位色填充
    '059.0': [(210, 1465, 785, 1700)],   # 标签两行，OCR 只框到一部分
    # 下面两处是压在红色喷溅上的艺术字，用掩码擦除：只重绘近灰阶像素，红色不动
    '004.1': [(162, 1272, 930, 1976)],
    '005.0': [(63, 1533, 897, 2191)],
    '066.1': [(240, 498, 1015, 828)],
}
MASK_ERASE = {'004.1', '005.0'}
erase_log = {}
FONT_MAX = {
    '004.1': 150, '005.0': 130, '013.0': 76,  '026.0': 92,  '059.0': 76,
    '074.0': 150, '055.3': 90,  '015.0': 58,  '050.1': 58,  '065.6': 56,
    '083.0': 66,  '066.1': 120,
}
EXTRA = {
    # 004 把「아아」惨叫和「키스12!!」呼喊聚成一块；惨叫保留，只译呼喊
    '004': [((162, 1272, 930, 1976), '키스12!!')],
    # 和「샹!!」聚在同一个尖角气泡里，OCR 只读到前半句
    '066': [((240, 498, 1015, 828), '밀어붙여!!')],
}
zh = {}
